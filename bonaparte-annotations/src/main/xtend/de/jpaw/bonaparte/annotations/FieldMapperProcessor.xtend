...
class FieldMapperProcessor extends AbstractMethodProcessor {
    ...
    override doTransform(MutableMethodDeclaration method, extension TransformationContext context) {
        ...
        // Remove the annotation from generated code, this saves a dependency
        method.removeAnnotation(method.annotations.findFirst[annotationTypeDeclaration === FieldMapper.newTypeReference.type])

        val oldBody = method.body
        method.body = [ '''
            // start of autogenerated code (by @FieldMapper)
            «buildMapping(dst.type, src.type, dst.simpleName, src.simpleName, true)»
            // end of autogenerated code (by @FieldMapper)
            «oldBody»
        ''' ]
    }
