...
public class JsonListTest {
    ...
    public void runBonaList() throws Exception {
        ...
        Assertions.assertEquals(j2, expected2);
        ...
        myList.add(t1);
        Assertions.assertEquals(JsonComposer.toJsonString(myList), "[" + expected1 + "]");

        // list related tests: two element list (separator expected)
        myList.add(t2);
        Assertions.assertEquals(JsonComposer.toJsonString(myList), "[" + expected1 + "," + expected2 + "]");
