        ...
        if (t == null) {
            ...
            if (seconds != 0 || millis != 0) {
                ...
                if (di.getHhmmss()) {
                    ...
                    int tmp = seconds / 60; // number of minutes
                    tmp = ((tmp / 60) * 100) + (tmp % 60);
                    writeVarInt(millis + (tmp * 100000) + (seconds % 60) * 1000);
                } else {
                    writeVarInt(millis + seconds * 1000);
                }
            }
            // then integral part
            writeVarInt(DayTime.dayAsInt(t.toLocalDate()));
        }
    }
    @Override
