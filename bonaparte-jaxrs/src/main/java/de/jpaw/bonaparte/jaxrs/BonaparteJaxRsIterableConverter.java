...
public class BonaparteJaxRsIterableConverter implements MessageBodyWriter<Iterable<BonaPortable>> {
    private final Logger LOGGER = LoggerFactory.getLogger(BonaparteJaxRsIterableConverter.class);

    @Override
    public long getSize(Iterable<BonaPortable> obj, Class<?> cls, Type type, Annotation[] anno, MediaType mediaType) {
    ...
    public boolean isWriteable(Class<?> cls, Type type, Annotation[] anno, MediaType mediaType) {
        ...
        return ByteArrayComposer.MIME_TYPE.equals(mediaType.toString()) && Iterable.class.isAssignableFrom(cls);
    }

    @Override
    public void writeTo(Iterable<BonaPortable> obj, Class<?> cls, Type type, Annotation[] anno, MediaType mediaType, MultivaluedMap<String, Object> args,
